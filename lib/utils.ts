...
//verify URI for useEffect
export function transformURI(value: string): string | undefined {
  const baseUrl = 'https://ipfs.kleros.io'
  if (value === undefined || value === null) {
    return undefined
  }

  const position = value.indexOf('/ipfs/')
  if (position !== -1) {
...
export const conciseDid = (addr: string) => {
  if (!addr || !addr.substring) return ''
  return (
    addr.substring(6, 11) + '...' + addr.substring(addr.length - 4, addr.length)
  )
}
